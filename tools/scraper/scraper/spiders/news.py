...
# def _parse_attribute(response, selector_type, selector):
#     attribute = ' '
...
class NewsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        news_page_link = _parse_attribute(
            response,
        ...
        if news_page_link:
            ...
            self.news_thumbnail_link = _parse_attribute(
                response,
    ...
    def parse_item(self, response, index):
        ...
        if parser_title:
            item['title'] = ''.join(
                _parse_attribute(response, parser_title.selector_type, parser_title.selector).getall())
        else:
            item['title'] = parser_title

        if parser_content:
            item['content'] = ''.join(
                _parse_attribute(response, parser_content.selector_type, parser_content.selector).getall())
        else:
            item['content'] = parser_content

        if parser_author:
            item['author'] = ''.join(
                _parse_attribute(response, parser_author.selector_type, parser_author.selector).getall())
        else:
            item['author'] = parser_author

        if parser_category:
            item['category'] = ''.join(
                _parse_attribute(response, parser_category.selector_type, parser_category.selector).getall())
        else:
            item['category'] = parser_category

        if parser_tag:
            item['tag'] = ''.join(
                _parse_attribute(response, parser_tag.selector_type, parser_tag.selector).getall())
        else:
            item['tag'] = parser_tag

        if parser_summary:
            item['summary'] = ''.join(
                _parse_attribute(response, parser_summary.selector_type, parser_summary.selector).getall())
        else:
            item['summary'] = parser_summary

        if parser_image_link:
            item['image_link'] = ''.join(
                _parse_attribute(response, parser_image_link.selector_type, parser_image_link.selector).getall())
        else:
            item['image_link'] = parser_image_link

        if parser_published_at:
            item['published_at'] = ''.join(_parse_attribute(
                response,
